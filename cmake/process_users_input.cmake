...
option(TESTING_GENERATED "Include kernel unit tests as generated by yateto" OFF)
...
if (NOT ${DEVICE_BACKEND} STREQUAL "none")
    ...
    if (${DEVICE_CODEGEN} STREQUAL "auto")
        ...
        set(AUTO_DEVICE_CODEGEN_BACKEND "none")
        ...
        if (${DEVICE_BACKEND} STREQUAL "oneapi" AND ${DEVICE_VENDOR} STREQUAL "intel")
            ...
            if (tinytc_FOUND AND tinytc_sycl_FOUND)
                if ((AUTO_DEVICE_CODEGEN_BACKEND STREQUAL "none") OR (AUTO_DEVICE_CODEGEN_BACKEND STREQUAL "default"))
                    message(STATUS "Intel GPUs are enabled, adding Tiny Tensor Compiler")
                    list(APPEND AUTO_DEVICE_CODEGEN "tinytc")
                    set(AUTO_DEVICE_CODEGEN_BACKEND "default")
                else()
                    message(STATUS "Intel GPUs are enabled; Tiny Tensor Compiler was found, but is ignored due to a different code generator taking precedence.")
                endif()
            endif()
        endif()

        execute_process(COMMAND "${Python3_EXECUTABLE}" -c "import tensorforge; tensorforge.print_cmake_path()"
                        OUTPUT_VARIABLE TENSORFORGE_PATH ERROR_QUIET)
        set(CMAKE_PREFIX_PATH "${TENSORFORGE_PATH}" ${CMAKE_PREFIX_PATH})
        find_package(TensorForge QUIET)
        if (TensorForge_FOUND)
            if ((AUTO_DEVICE_CODEGEN_BACKEND STREQUAL "none") OR (AUTO_DEVICE_CODEGEN_BACKEND STREQUAL "tensorforge"))
                message(STATUS "GPUs are enabled, adding TensorForge")
                list(APPEND AUTO_DEVICE_CODEGEN "tensorforge")
                set(AUTO_DEVICE_CODEGEN_BACKEND "tensorforge")
            else()
                message(STATUS "GPUs are enabled; TensorForge was found, but is ignored due to a different code generator taking precedence.")
            endif()
        endif()

        # generic device GEMM tool
        execute_process(COMMAND "${Python3_EXECUTABLE}" -c "import gemmforge; gemmforge.print_cmake_path()"
                        OUTPUT_VARIABLE GEMMFORGE_PATH ERROR_QUIET)
